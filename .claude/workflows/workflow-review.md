...
- [ ] If ALREADY on the target branch (PR branch, requested branch name, or the branch already checked out for review) →roceed with analysis on current branch
- [ ] If DIFFERENT branch than the review target →ffer to use worktree: "Use git-worktree skill for isolated Call `skill: git-worktree` with branch name
...
4. **Track Progress**:
   - Rename file when status changes: pending →eady →omplete
